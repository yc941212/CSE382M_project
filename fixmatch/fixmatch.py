...
class FixMatch(CTAReMixMatch):
    ...
    def model(self, batch, lr, wd, method, wu, confidence, uratio, ema=0.999, **kwargs):
        ...
        if method == "CEhard":
            loss_xeu = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.argmax(pseudo_labels, axis=1), logits=logits_strong)
        if method == "CEsoft":
            loss_xeu = tf.nn.softmax_cross_entropy_with_logits_v2(labels = pseudo_labels, logits = logits_strong)
        if method == "l2":
            loss_xeu = tf.square(tf.norm(pseudo_labels - tf.nn.softmax(logits_strong), axis = 1))
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string('method', 'CEhard', 'regularization method.')
